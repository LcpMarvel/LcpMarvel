...
<!-- INTRO:START -->
平时主要用 Rust 写工具，大多是围绕现成数据的小库：修复坏掉的 JSON、安全读写 Steam 本地配置、清洗文档解析结果、提取 Dota 2 地图。这些工具的共同原则是不丢数据、不擅自改动，出问题宁可降级也不要崩。最近另一条线是 agent 工具链——让 Claude Code 顺手接上 GLM，或者让 agent 画飞书白板时真的讲究点设计。
<!-- INTRO:END -->
