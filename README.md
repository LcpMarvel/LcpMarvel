...
<!-- INTRO:START -->
主要用 Rust 做工具，方向是稳妥地处理现实世界的脏数据：JSON 修复、文档解析结果的后处理、游戏本地配置和地图资源，共同的原则是不丢数据、不凭空添加内容，出问题时宁可 fail-open 也不搞崩上游。另一条线是 AI agent 周边，包括 Claude Code 的可移植封装，以及让 agent 产出真正有设计感的飞书白板这类技能。
<!-- INTRO:END -->
