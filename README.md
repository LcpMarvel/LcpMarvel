...
<!-- INTRO:START -->
平时主要写 Rust 工具，大多在做同一类事：安全地处理不太可靠的数据——修复损坏的 JSON、整理文档解析结果、读写 Steam 和 Dota 2 的本地文件，原则是不丢数据、不添一字、出了问题也不崩上游。最近在折腾 Claude Code 的 agent 生态，顺手做了些 GLM 接入和飞书白板生成相关的周边。
<!-- INTRO:END -->
